"""
main.py — Real-time sign language detection with audio output.
...
from utilities import (
    ACTIONS, MODEL_PATH, SEQUENCE_LENGTH, THRESHOLD, KHMER_SPEECH, AUDIO_PATH,
...
# Map action name → wav file path
AUDIO_FILES = {
    action: os.path.join(AUDIO_PATH, f"{action}.wav")
    for action in ACTIONS
}


def speak(action):
    if not speech_enabled[0] or action == "nothing":
        return
    wav = AUDIO_FILES.get(action)
...
# ── Verify audio files ─────────────────────────────────────────────────────────
for action, wav in AUDIO_FILES.items():
    status = "OK" if os.path.exists(wav) else "MISSING"
    print(f"  Audio [{status}] {action} → {wav}")
...
with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
    while cap.isOpened():
        ...
        if key in (ord("q"), 27):
            ...
            if display_word not in ("No sign", "nothing"):
                sentence.append(display_word)
                speak(display_word)
                print(f"\nAccepted: {display_word}  ({KHMER_SPEECH.get(display_word, '')})")
                sequence.clear()
                smoother.reset()
                display_word = "No sign"
                confidence   = 0.0
