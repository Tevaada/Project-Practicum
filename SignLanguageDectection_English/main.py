...
from utils import (
    ACTIONS, MODEL_PATH, SEQUENCE_LENGTH, THRESHOLD,
    mp_holistic, mediapipe_detection, draw_styled_landmarks,
    extract_keypoints, PredictionSmoother,
)

# ── Speech via Windows PowerShell TTS (no pyttsx3 threading issues) ────────────
speech_enabled = [True]   # list so it's mutable across scopes


def speak(text):
    """Fire-and-forget TTS using PowerShell — works every time on Windows."""
...
with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
    while cap.isOpened():
        ...
        cv2.imshow("Sign Detection",
                   draw_panel(image, display_word, confidence,
                               sentence, last_probs))
