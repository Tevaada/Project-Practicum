...
def wait_for_ready(cap, holistic, action, seq):
    """Show a countdown screen and wait for user to get ready."""
    for countdown in range(3, 0, -1):
        ...
        while cv2.getTickCount() < deadline:
            ...
            h, w = image.shape[:2]
            # dark overlay
            cv2.rectangle(image, (0, 0), (w, h), (0, 0, 0), -1)
            cv2.addWeighted(image, 0.0, frame, 0.5, 0, image)
...
with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
    ...
    for action in ACTIONS:
        ...
        for seq in range(NO_SEQUENCES):
            ...
            # skip already-complete sequences
